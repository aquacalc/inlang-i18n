...
// calc critical pH at which UIA-N becomes unsafe
// for a given TA-N, temp, sal, & critical UIA-N
// from Fivelstad formula
export const calcCritPhTan = (tan, unIonized, temp, sal) => {
  // console.log(`TA-N ${tan}, UIA-N ${unIonized}, temp ${temp}, sal ${sal}`)
...
// in the definition for the total and the seawater pH scale are molal concentrations: i.e. mol/kg-H2O.
// But in Roy et al. [1993b, p. 257] and in DOE [1994, chap.. 4, SOP 6, p. 1] ... concentrations
// for the seawater and total pH scale are molin, i.e. mol/kg-soln. To be consistent with DOE [1994] and
// Dickson et al. [2007] mol/kg-soln is chosen here for the free, total, and seawater scale[s]."
...
// [NB] icTemp & icSal
export const phFreeToPhTot = (phFree, sal, temp, p) => {
  const phTot = phFree - Math.log10(ahFreeToTotFactor(sal, temp, 0));
};
...
export const calcBorate = (ph, t, sal) => {
  ...
  // console.log(`Borate conc: ${concB}`)

  let myKB = calcBorateFactor(t, sal);

  // console.log(`Borate conc: ${concB}`)
  // console.log(`Borate Kb: ${myKB}`)
...
export const calcHCO3 = (dic, t, s, ph) => {
  let bicarbonate = dic * alphaOne(t, s, ph);
...
// calc pH for given Alk & DIC

// #pragma mark - pH of [Alk] & DIC
// # ** NB: Use the secant method to solve the resulting quartic polynomial
export const calcPhForAlkDic = (alk, dic, t, s) => {
  // # ** assemble the Ki and concentrations...
  const KW = calcKWMehrbach(t, s);
  const K1 = getK1(t, s, 0.0);
  const K2 = getK2(t, s, 0.0);
  const B = calcBorateConcOfSalinity(s);
  const KB = calcBorateFactor(t, s);

  // # ** calculate the coefficients of the quartic polynomial...
  const q4 = 1 - KW;
  const q3 = alk - KW * KB + KB - K1 * KW + K1;
  const q2 =
    alk * KB -
    KB * B +
    alk * K1 -
...
export const calcPhTotForOmegaAr = (omega, dic, ca, t, s, p) => {
  ...
  root = -Math.log10(root);
  // cat("Ca-secant pH: ", thePh, " vs quadForm pH: ", root, '\n\n')
  // console.log(`Secant: ${thePh} vs. Quadratic: ${root}`);
...
export const calcDicOfAlkPh = (alk, ph, temp, sal) => {
  const m = phLineSlope(temp, sal, ph);
  // console.log(`===============`);
  // console.log(`${alk} (${typeof alk})`);
  // console.log(`${ph} (${typeof ph})`);
  // console.log(temp);
  // console.log(`${sal} (${typeof sal})`);
  // console.log(m);
  // console.log(calcHydroxide(ph, temp, sal));
  // console.log(calcBorate(ph, temp, sal));
  // console.log(calcHydronium(ph));
  // console.log(`===============`);
...
// [NB] Need DIC from [Alk-low, CO2*] & [Alk-high, CO2*] to define enhanced Green Zone
// #pragma mark - DIC of [Alk] & [CO2]
// # ** NB: Use the secant method to solve the resulting quartic polynomial
export const calcDicForAlkCo2 = (alk, co2, t, s) => {
  // # ** assemble the Ki and concentrations...
  const KW = calcKWMehrbach(t, s);
  const K1 = getK1(t, s, 0.0);
  const K2 = getK2(t, s, 0.0);
  ...
  const q4 = 1;
  const q3 = KB + alk;
  const q2 = alk * KB - co2 * K1 - KB * B - KW;
  const q1 = -co2 * KB * K1 - co2 * 2 * K1 * K2 - KW * KB;
  const q0 = -2 * co2 * KB * K1 * K2;

  // Compute the pH of co2-crit for alk, t, & s (& p = 0 bar)
  // console.log(`${q0} - ${q1} - ${q2} - ${q3} - ${q4}`);
  let thePh = calcSecantMethod(q4, q3, q2, q1, q0);
  thePh = -Math.log10(thePh);

  const dic = calcDicOfAlkPh(alk, thePh, t, s);

  // console.log(`Critical pH for CO2 = ${co2} mol/kg: ${thePh} (which Scale??)`);
  // console.log(`(${dic * 1000}, ${alk * 1000})`);
  // console.log(` `);

  return dic * 1000;
  // return {x: dic * 1000, y: alk * 1000};
};
...
export const calcLatentHeat = (temp, sal) => {
  ...
  const a = [
    2500899.1412, -2369.1806479, 0.26776439436, -0.0081027544602,
    -0.000020799346624,
  ];
...
// ** Find the pH (FREE) for the given alk & dic at temp t and salinity s **
// Alternative function to calc pH from [Alk] & DIC
// Why? Secant method (above) fails to find pH > ~8
// Use? For WQ Map when displaying pH for dragged target-waypoint
// ...a little round-ing help for testing...

// NB: When needed, over-ride round() in generalConversions
const round = (value, decimals) =>
  Number(Math.round(value + "e" + decimals) + "e-" + decimals);

export const findPhFromAlkDic = (alk, dic, t, s) => {
  // Initialize guess phFree_guess, tolerance tol, iterations k, & max iterations n
  let phFree_guess = 7;
  const tol = 0.000001;
  let k = 0;
  const n = 7000;

  // [NB] Refine initial guess for 'upper-left' of WQ Map
  if (dic * 1000 <= 2 && alk * 1000 >= 4) {
    phFree_guess = 11;
  }

  const ph_step = 0.001;

  // Use guess to calculate DIC for given alk, t, s
  let dic_est = calcDicOfAlkPh(alk, phFree_guess, t, s);

  // Compare estimated DIC with given DIC
  let diff = dic_est - dic;

  // While diff > tol, increment phFree_guess & recalc dic_est
  //       until reach solution or exceedmax iterations n
  while (Math.abs(dic_est - dic) > tol && k <= n) {
    // Change phFree_guess
    // If estimated DIC > given DIC, then INCREMENT ph guess
    phFree_guess =
      dic_est - dic > 0 ? (phFree_guess += ph_step) : (phFree_guess -= ph_step);

    dic_est = calcDicOfAlkPh(alk, phFree_guess, t, s);

    // console.log(`${k}: ${dic_est * 1000}`);
  ...
  // console.log(
  //   `${k}: dic_est: ${round(dic_est * 1000, 2)} vs. dic: ${round(
  //     dic * 1000,
  //     2
  //   )} => pH ${round(phFree_guess, 2)}`
  // );

  return phFree_guess;

  // if (!isNaN(phFree_guess)) {
  //   const phNbs = 8.8888;
  // }
};
...
export const calcSecantMethod = (q4, q3, q2, q1, q0) => {
  ...
  // console.log(q0);
  // console.log("------------");
  // console.log('IN SECANT METHOD...')
  // let myAns = polyroot(c(q0, q1, q2, q3, q4))
  // console.log(myAns)
  // console.log('Evo')
  // console.log("------------");
  // ***************************************
